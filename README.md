...
### Part 1.1
In JAX-RS, a new instance of each resource class gets created for every request that comes in. Because of that, if I stored my room or sensor data directly inside the resource class, it would be lost after each request since the object is discarded. To avoid this, I created a separate DataStore class to hold all the data using static fields. Since static fields belong to the class itself rather than individual instances, the data is shared across all requests, no matter how many resource objects are created. I also chose to use ConcurrentHashMap instead of a regular HashMap because multiple requests can happen at the same time. A regular HashMap isn’t safe in that situation and can become corrupted if two requests try to modify it at once, whereas ConcurrentHashMap handles this safely.

### Part 1.2
HATEOAS stands for Hypermedia as the Engine of Application State. Simply, it means that API responses include links that guide the client on what it can do next, instead of the client needing to already know all the URLs. For example, my discovery endpoint returns links to /api/v1/rooms and /api/v1/sensors, so a developer using the API for the first time can immediately see what’s available. This is more useful than static documentation because the API effectively documents itself. If the URLs ever change, the links in the responses update as well, so clients don’t break. Overall, it makes things much easier for anyone building on top of the API.
